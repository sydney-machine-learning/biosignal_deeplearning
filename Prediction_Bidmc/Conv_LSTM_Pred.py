...
ch1 = ModelCheckpoint('model_conv_lstm_1.h5', verbose=1, monitor='val_loss',save_best_only=True, mode='auto')
ch2 = EarlyStopping(monitor="val_loss",patience=150,verbose=0,mode="min")
model = Sequential()
model.add(ConvLSTM2D(filters=64, kernel_size=(1,3), activation='relu', input_shape=(n_steps, 1, n_length, n_features),padding='same', return_sequences=True))
